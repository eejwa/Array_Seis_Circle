...
All_Noise_arr = np.load(numpy_dir + "All_Noise_%s_%s_%s_array.npy" %(Boots, fmin, fmax))
All_Thresh_Peaks_arr = np.load(numpy_dir + "All_Thresh_Peaks_%s_%s_%s_array.npy" %(Boots, fmin, fmax))
...
newlines = cu.create_newlines(st, file_path=Results_filepath, phase=phase, window=[t_min, t_max], Boots=Boots,
                              epsilon = epsilon, slow_vec_error=3, Filter=False)
...
with PdfPages(Res_dir + f"Clustering_Summary_Plot_{fmin:.2f}_{fmax:.2f}.pdf") as pdf:
    ...
    p.plot_clusters_XY(labels=labels, tp=Lin_Mean, peaks=All_Thresh_Peaks_arr,
                       ...
                       sstep=s_space, title="Clusters", predictions=predictions,
                       ellipse=True, std_devs=[1,2,3])
    ...
    p = plotting(ax=ax2)
    p.plot_record_section_SAC(st=st, phase=phase, tmin=cut_min, tmax=cut_max, align=True)

    ax2.vlines(x = t_min, ymin = np.amin(distances),
               ymax = np.amax(distances),
               label = 'window min')

    ax2.vlines(x = t_max, ymin = np.amin(distances),
               ymax = np.amax(distances),
               label = 'window min')
